...
# Delete rows 79 and downwards to clean up the mess
max_r = ws.max_row
if max_r >= 79:
...
tasks_to_add = [
    ...
    ['', '', 'Tenancy Hierarchical Model & RLS Implementation', 'Completed', '1'],
    ['', '', 'Delegated RBAC & Tenancy Documentation', 'Completed', '1'],
    # Phase 12 header
...
# We will copy styling from row 74 (a standard 'Completed' task) for tasks
# We will copy styling from row 73 (a Phase header) for the Phase 12 header
# We will copy styling from row 78 (a WIP task) for the Risk task. Wait, row 78 is Risk: PagerDuty. Let's use it.
# Wait, are they still there? Yes, 73, 74, 75, 76, 77, 78 are untouched.
...
for idx, data in enumerate(tasks_to_add):
    ...
    for col in range(1, 10): # Copy styling for first 9 columns
...
# Special handling for "Not started" color? 
# The original was "No started" with a red color. I will use openpyxl PatternFill to color the status cell red.
...
wb.save(file_path)
print("Excel file fully reconstructed, standardised, and saved.")
