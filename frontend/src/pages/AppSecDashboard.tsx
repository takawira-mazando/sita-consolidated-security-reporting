  ...
  return (
    <div className="dash">
      ...
      <div className="stats-bar">
        ...
        <StatCard ghost="Fix Rate" accent="var(--green)" value="—" valueColor="var(--text-muted)" label={<><span>Fix Rate (30d) </span><OemTag source="appscan" label="not tracked" /></>} delta="backend has no fix rate" deltaColor="var(--text-muted)" />
        <StatCard ghost="WAF" accent="var(--red)" value="—" valueColor="var(--text-muted)" label={<><span>WAF Blocks </span><OemTag source="imperva-waf" label="not tracked" /></>} delta="backend has no WAF feed" deltaColor="var(--text-muted)" />
      </div>
      ...
      <div className="dash-grid cols-2-1">
        <Panel title={<><span>API Exposure</span> <OemTag source="api-sec" label="API Security" /></>} bodyClassName="scroll-x" bodyStyle={{ padding: 0 }}>
          <table>
            <thead><tr><th>API Endpoint</th><th>Risk</th><th>Auth</th><th>Rate Limit</th><th>Shadow</th></tr></thead>
            <tbody>
              <tr><td colSpan={5}><div className="panel-empty">API Security exposure feed not connected yet.</div></td></tr>
            </tbody>
          </table>
        </Panel>
        <Panel title={<><span>WAF Block Summary</span> <OemTag source="imperva-waf" label="Imperva WAF" /></>}>
          <div className="panel-empty">Imperva WAF telemetry not connected yet.</div>
        </Panel>
      </div>
